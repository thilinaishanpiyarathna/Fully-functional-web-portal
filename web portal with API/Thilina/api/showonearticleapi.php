...
if ($apikey->num_rows > 0) {
    ...
    $articleid=$_GET["aid"];
    $sql = "SELECT * FROM entry WHERE ID=$articleid";
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            ...
            $data=array_push_assoc($data,'topic',$row["title"]);
            $data=array_push_assoc($data,'keys',$row["keywords"]);
            $data=array_push_assoc($data,'doc',$row["text"]);
        }
